-- ============================================================
-- Migration: Pricing Module Integrity (T2 + T3 + T4)
-- Date: 2026-02-12
-- Description:
--   T2: Prevent NULL precio_actual (causes POS errors)
--   T3: Add FOR UPDATE to sp_aplicar_precio (prevent race conditions)
--   T4: Add index on precios_proveedor(codigo_barras)
-- ============================================================

-- T2: Backfill NULLs and add NOT NULL constraint
UPDATE productos SET precio_actual = 0 WHERE precio_actual IS NULL;

ALTER TABLE productos
  ALTER COLUMN precio_actual SET DEFAULT 0,
  ALTER COLUMN precio_actual SET NOT NULL;

-- T3: Surgical fix - add FOR UPDATE to prevent concurrent overwrites
...
-- T4: Index for barcode lookups on supplier prices
