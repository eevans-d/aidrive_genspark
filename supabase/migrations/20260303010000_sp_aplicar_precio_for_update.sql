-- Migration: Add SELECT ... FOR UPDATE to sp_aplicar_precio
-- Purpose: Prevent race condition where concurrent price applications
--          read stale margin data and bypass minimum margin validation.
-- Rollback: Re-deploy previous version without FOR UPDATE clause.
...
BEGIN
  ...
  -- FOR UPDATE: lock the product row to prevent concurrent stale reads
