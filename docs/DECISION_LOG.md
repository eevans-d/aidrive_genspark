...
**Última actualización:** 2026-02-22 (verificación independiente D-154)
...
| D-007 | `precios_proveedor` se usa exclusivamente para scraping (Maxiconsumo/locales); precios de compra internos NO se persisten hasta definir modelo dedicado | Cerrada (fix D-153) | 2026-01-10 | Se eliminó el insert desalineado en `POST /deposito/ingreso` que intentaba columnas inexistentes (`proveedor_id`, `producto_id`, `precio`, `fecha_actualizacion`). La tabla `precios_proveedor` conserva su esquema de scraping. Persistencia de precios de compra diferida como feature futura. Fix: `supabase/functions/api-minimarket/index.ts:1643-1648`. Evidencia original: `supabase/migrations/20260109060000_create_precios_proveedor.sql`. |
...
| D-010 | API proveedor es interna: auth definitiva por `x-api-secret` (timing-safe, min 32 chars) + origin allowlist interna | Cerrada (definitiva D-153) | 2026-01-11 | El esquema `x-api-secret` se formaliza como definitivo para este servicio server-to-server. Controles implementados: comparación timing-safe, longitud mínima 32 chars, allowlist de orígenes configurable via `INTERNAL_ORIGINS_ALLOWLIST`, server-to-server sin Origin permitido. Riesgos residuales aceptados: secret compartido (mitigado por rotación documentada D-076), sin rate-limit dedicado (cubierto por gateway). No se requiere migración a OAuth/mTLS dado el volumen y el perfil interno del servicio. Evidencia: `supabase/functions/api-proveedor/utils/auth.ts`, `docs/SECRET_ROTATION_PLAN.md`. |
...
| D-058 | **Idempotency en reservas** | Completada (normalizada D-153) | 2026-02-04 | El estado parcial histórico quedó superado: la migración de `idempotency_key` y su índice existen en repo, el handler `/reservas` usa `Idempotency-Key`, y el estado DB quedó sincronizado (`44/44`) en cierres posteriores. Evidencia: `supabase/migrations/20260204100000_add_idempotency_stock_reservado.sql`, `supabase/functions/api-minimarket/handlers/reservas.ts`, `docs/closure/VALIDACION_POST_REMEDIACION_2026-02-17.md`. |
| D-059 | **Locks distribuidos para cron jobs** | Completada (normalizada D-153) | 2026-02-04 | El estado parcial histórico quedó superado: `cron_jobs_locks` y RPC `sp_acquire_job_lock`/`sp_release_job_lock` están en migraciones y el orquestador cron las invoca. Evidencia: `supabase/migrations/20260204110000_add_cron_job_locks.sql`, `supabase/functions/cron-jobs-maxiconsumo/orchestrator.ts`, `docs/closure/VALIDACION_POST_REMEDIACION_2026-02-17.md`. |
| D-060 | **Reserva atómica vía RPC** | Completada (normalizada D-153) | 2026-02-04 | El estado parcial histórico quedó superado: `sp_reservar_stock` existe en migraciones y el endpoint `/reservas` lo consume con idempotencia; el drift DB histórico fue cerrado en validaciones posteriores. Evidencia: `supabase/migrations/20260204120000_add_sp_reservar_stock.sql`, `supabase/functions/api-minimarket/handlers/reservas.ts`, `docs/closure/OPEN_ISSUES.md`. |
...
| D-082 | **Gate 16: Sentry integrado, DSN pendiente del owner** | Parcial (histórico, supersedido por D-100) | 2026-02-12 | Estado de transición previo al cierre definitivo D-100. Evidencia histórica: `docs/closure/EVIDENCIA_GATE16_2026-02-12.md`. |
...
| D-099 | **Gate 16 revalidado post-correccion DSN: ingest tecnico OK (`200`)** | Parcial (histórico, supersedido por D-100) | 2026-02-14 | Revalidación técnica previa al cierre visual/operativo definitivo registrado en D-100. Referencia: `docs/closure/EVIDENCIA_GATE16_2026-02-14.md`. |
...
| D-150 | **Pulido documental final + integración explícita de trabajos paralelos Claude + cierre de gobernanza** — (1) Pasada DocuGuard de detalle sobre docs principales para eliminar drift puntual de conteos y mejorar claridad operativa. (2) Integración explícita de continuidad Claude Code (D-146/D-147/D-148) en README principal, runbook de prompts y reporte de ejecución documental. (3) Gobernanza documental cerrada: creación de `CONTRIBUTING.md` y `CODE_OF_CONDUCT.md`. (4) Consolidación del orquestador en fuente canónica `.ini` y archivo alias adaptado sin duplicación operativa. (5) Resultado: docs principales alineadas al FactPack vigente y repositorio documental sin pendientes críticos de estructura. | Completada | 2026-02-21 | Evidencia: `README.md`, `docs/AGENTS.md`, `docs/ESTADO_ACTUAL.md`, `docs/closure/# PROMPTS EJECUTABLES PARA DOCUMENTACIÓN.ini`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `docs/closure/REPORTE_EJECUCION_DOCUMENTAL_PRODUCCION_2026-02-21.md`. |
| D-151 | **Auditoría de pendientes vigentes + depuración documental controlada** — (1) Revisión DocuGuard de pendientes reales y procesos operativos aún no automatizados. (2) `OPEN_ISSUES` actualizado con pendientes vigentes explícitos y snapshot técnico normalizado (histórico remoto 13 vs FactPack local 14). (3) Índice canónico de `README.md` actualizado para incluir `CONTRIBUTING.md` y `CODE_OF_CONDUCT.md`. (4) Alias `PROMPTS_EJECUTABLES_DOCUMENTACION_ADAPTADA_2026-02-21.md` marcado como deprecado para futura limpieza sin pérdida de trazabilidad. (5) Se emitió reporte de auditoría con candidatas a deprecación/eliminación. | Completada | 2026-02-22 | Evidencia: `docs/closure/AUDITORIA_PENDIENTES_Y_DEPURACION_DOCS_2026-02-22.md`, `docs/closure/OPEN_ISSUES.md`, `README.md`, `docs/closure/PROMPTS_EJECUTABLES_DOCUMENTACION_ADAPTADA_2026-02-21.md`. |
| D-152 | **Auditoría intensiva de pendientes ocultos + diseño de prompt de ejecución Claude Code** — (1) Barrido intensivo DocuGuard en canónicos + histórico cercano para detectar pendientes no explícitos en `OPEN_ISSUES`. (2) Hallazgos ocultos priorizados: decisiones históricas con estado `Parcial/Bloqueada` no mapeadas de forma operacional (D-007, D-010, D-058/059/060, D-082/D-099), duplicación de pendiente FAB en `OPEN_ISSUES`, item corregido no tachado en issues técnicos, y necesidad de ruta de cierre para alias documental de prompts. (3) Snapshot canónico actualizado a FactPack 2026-02-22 (`docs=204`, `functions=14`, `skills=22`, links PASS). (4) Se genera contexto `prompt engineering` de alta precisión para nueva ventana Claude Code con fases, gates y entregables de cierre. | Completada | 2026-02-22 | Evidencia: `docs/closure/AUDITORIA_INTENSIVA_PENDIENTES_OCULTOS_2026-02-22.md`, `docs/closure/CONTEXT_PROMPT_ENGINEERING_CLAUDE_CODE_AUDITORIA_INTENSIVA_PENDIENTES_OCULTOS_2026-02-22.md`, `docs/closure/OPEN_ISSUES.md`, `README.md`. |
| D-153 | **Cierre técnico-documental D-153: D-007 fix + D-010 definitiva + FAB exclusión formal + smoke nightly** — (1) D-007 cerrada: eliminado insert desalineado en `POST /deposito/ingreso` que intentaba columnas inexistentes en `precios_proveedor`; precio de compra diferido como feature futura. (2) D-010 cerrada: esquema `x-api-secret` formalizado como definitivo (timing-safe, min 32 chars, origin allowlist, rotación documentada). (3) FAB en `/pos`/`/pocket` excluido formalmente: rutas standalone por diseño para workflows de foco (caja/scanner); FAB podría interferir con touch targets y scanner race lock. (4) Smoke de seguridad nightly operativizado: `.github/workflows/security-nightly.yml` creado (cron 04:00 UTC, `RUN_REAL_TESTS=true`). (5) Docs canónicas sincronizadas: `DECISION_LOG`, `OPEN_ISSUES`, `ESTADO_ACTUAL` actualizados con evidencia. | Completada | 2026-02-22 | Evidencia: `supabase/functions/api-minimarket/index.ts:1643-1648`, `supabase/functions/api-proveedor/utils/auth.ts`, `.github/workflows/security-nightly.yml`, `docs/closure/REPORTE_CIERRE_EXHAUSTIVO_D153_2026-02-22.md`. |
| D-154 | **Verificación independiente post D-153 (realidad operativa vs cierre documental)** — Se validó de forma externa al reporte D-153 el estado de producción real. Resultado: cierre técnico en repo confirmado, pero quedan acciones operativas para producción: (1) merge de rama `docs/d150-cierre-documental-final` en `main` (ahead `0/5`), (2) deploy de `api-minimarket` con fix D-007, (3) deploy de `backfill-faltantes-recordatorios` (presente en repo, ausente en remoto), (4) configurar variables repo `VITE_SUPABASE_URL` y `VITE_SUPABASE_ANON_KEY` usadas por `security-nightly.yml`, (5) configurar secret `SUPABASE_DB_URL` requerido por `backup.yml`. | Completada | 2026-02-22 | Evidencia: `git rev-list --left-right --count main...docs/d150-cierre-documental-final` (`0 5`), `supabase functions list --project-ref <ref>`, `gh variable list -R eevans-d/aidrive_genspark`, `gh secret list -R eevans-d/aidrive_genspark`, `.github/workflows/security-nightly.yml`, `.github/workflows/backup.yml`. |
| D-155 | **Servicio OCR para facturas = Google Cloud Vision API** | Aprobada | 2026-02-23 | Opciones evaluadas: Google Cloud Vision (~$1.50/1000 pág, alta precisión), Claude Vision (costo por tokens), Tesseract self-hosted (gratis, menor precisión), AWS Textract (~$1.50/1000 pág). Decisión: GCV por precisión en documentos de baja calidad + robustez demostrada en facturas. Secret requerido: `GCV_API_KEY` en Supabase secrets. Costo estimado: ~$0.0015/factura. Contexto: Mega Plan O1 Tarea 0.0. |
...
| P2 | ~~Ejecutar smoke real de seguridad de forma periódica (`RUN_REAL_TESTS=true`) y dejar evidencia en `docs/closure/`~~ | ✅ CERRADO (D-153): `.github/workflows/security-nightly.yml` creado con cron 04:00 UTC |
