...
| D-007 | `precios_proveedor` se usa para scraping (Maxiconsumo/locales); precios de compra internos quedan como `precios_compra_proveedor` (pendiente) | Reabierta (DESINCRONIZADO, D-153) | 2026-01-10 | No existe tabla `precios_compra_proveedor` en migraciones y `POST /deposito/ingreso` intenta insertar columnas inexistentes en `precios_proveedor` (`proveedor_id`, `producto_id`, `precio`, `fecha_actualizacion`). Requiere definición final de modelo y fix de runtime. Evidencia: `supabase/functions/api-minimarket/index.ts:1645`, `supabase/migrations/20260109060000_create_precios_proveedor.sql`. |
...
| D-010 | API proveedor es interna: auth por shared secret + CORS allowlist; check por header es temporal y debe reemplazarse por verificacion real en FASE 7/8 | Vigente (hardening pendiente) | 2026-01-11 | La implementación actual mantiene `x-api-secret` + allowlist interna; la evolución de hardening sigue abierta como pendiente operativo/documental. Evidencia: `supabase/functions/api-proveedor/utils/auth.ts`, `supabase/functions/api-proveedor/index.ts`. |
...
| D-153 | **Revisión exhaustiva intensiva D-153 + prompt engineering de continuidad Claude Code** — (1) Revalidación profunda de pendientes ocultos con contraste código/migraciones/docs canónicas. (2) Normalización de estados históricos en `DECISION_LOG` (D-058/059/060 completadas; D-082/D-099 marcadas como históricas supersedidas por D-100). (3) Detección de pendiente técnico no visible en backlog: `POST /deposito/ingreso` intenta insertar columnas inexistentes en `precios_proveedor` cuando llega `precio_compra` + `proveedor_id` (riesgo operativo real). (4) `OPEN_ISSUES` y `ESTADO_ACTUAL` actualizados con pendientes vigentes reales y candidatos de deprecación documental. (5) Se emite nuevo contexto prompt de ejecución para ventana Claude Code orientado a cierre técnico/documental con gates. | Completada | 2026-02-22 | Evidencia: `docs/closure/AUDITORIA_EXHAUSTIVA_PENDIENTES_OCULTOS_D153_2026-02-22.md`, `docs/closure/CONTEXT_PROMPT_ENGINEERING_CLAUDE_CODE_CIERRE_EXHAUSTIVO_D153_2026-02-22.md`, `docs/closure/OPEN_ISSUES.md`, `supabase/functions/api-minimarket/index.ts:1645`, `supabase/migrations/20260109060000_create_precios_proveedor.sql`. |
