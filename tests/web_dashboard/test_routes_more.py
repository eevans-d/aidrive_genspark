...
def test_trends_months_clamp_and_filters_invalid():
    ...
    r = client.get("/api/trends?months=999&start_date=2025-99-99&end_date=2025-13-40&proveedor=@@X", headers={"X-API-Key": "test-key"})
...
def test_stock_timeline_days_clamp():
    # days=0 -> clamp a 1
    r = client.get("/api/stock-timeline?days=0", headers={"X-API-Key": "test-key"})
    assert r.status_code == 200
...
def test_weekly_sales_weeks_clamp():
    ...
    r = client.get("/api/weekly-sales?weeks=999", headers={"X-API-Key": "test-key"})
    assert r.status_code == 200
    assert isinstance(r.json(), list)
...
def test_stock_by_provider_fallback(monkeypatch):
    ...
    try:
        r = client.get("/api/stock-by-provider", headers={"X-API-Key": "test-key"})
        assert r.status_code == 200
        data = r.json()
